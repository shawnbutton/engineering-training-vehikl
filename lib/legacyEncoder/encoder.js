...
const encodePipeRepairQueryString = params => {
  ...
  return encodeURIComponent('{') + queryString + encodeURIComponent('}')
}

const encodeSpectrumRepairQueryString = object => {
  return encodeQuery(object)
}
...
const encodeQuery = object => {
  ...
  Object
    .keys(object)
    .forEach(function (key) { queryParts.push(encodeKeyValue(key, object[key])) })

  console.log(queryParts.join(','))

  Object
    .keys(object)
    .forEach(function (key) { query += encodeKeyValue(key, object[key]) })

  return encodeURIComponent('{') + removeTrailingComma(query) + encodeURIComponent('}')
}
