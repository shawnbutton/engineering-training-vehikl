...
const encodeQuery = (object, quotedValues, values) => {
  ...
  Object
    .keys(object)
    .forEach(function (key) { parts.push(encodePart(key, object[key], quotedValues)) })

  return encodeURIComponent('{') + parts.join(',') + encodeURIComponent('}')
}

const encodePart = (key, value, quoteValues) => {
